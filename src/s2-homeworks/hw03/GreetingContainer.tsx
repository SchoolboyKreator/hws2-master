...
// ВЕРНУТЬ ИВЕНТ
export const pureOnBlur = (name: string, setError: (errorMessage: string) => void) => {
    if (name.trim() === '') {
        setError("Ошибка! Введите имя!")
    }
}
...
const GreetingContainer: React.FC<GreetingContainerPropsType> = ({users, addUserCallback}) => {
    ...
    // console.log(setError('Ошибка! Введите имя!'))
    ...
    const onBlur = () => {
        pureOnBlur(name, setError) // исправление
    }
    ...
    const lastUserName: string | undefined = users.length > 0 ? users[users.length - 1].name : undefined;    // users[users.length].name // need to fix
